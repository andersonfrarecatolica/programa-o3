...
function constroiPagina(dados) {
  var htmlFinal = '<div class="col-xs-12">'; // string que vai conter todo o HTML
  ...
  $ajaxUtils.sendGetRequest(itensHtml, function(itensHtml) {
    for (var i = 0, max = dados.length; i < max; i++) {
	  ...
	  $('#menu_topo').append('<li class="text-center"><a href="#'+titulo+'">'+titulo+'</a></li>');
    }
    htmlFinal += '</div>';
